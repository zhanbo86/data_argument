#ifndef data_enhance_UTIL_UTIL_H_
#define data_enhance_UTIL_UTIL_H_
  ...
  static unsigned int levenshtein_distance(const T &s1, const T &s2) {
    ...
    for (unsigned int i = 0; i < len1; i++) {
      ...
      for (unsigned int j = 0; j < len2; j++)
        col[j + 1] = data_enhance::Utils::min(
            data_enhance::Utils::min(prevCol[1 + j] + 1, col[j] + 1),
...
#endif  // data_enhance_UTIL_UTIL_H_
