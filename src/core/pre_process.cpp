#include "data_enhance/core/pre_process.hpp"
...
void TextDetector::segmentRow(cv::Mat &open_src_in,cv::Mat &gray_src_in, cv::Mat &out,cv::Mat &out_gray, float gap=0.08)
{
    vector<int> pos_h;
    ...
    Mat charRow_gray(gray_src_in, mainRow);//modify on 0309
    out = charRow;
    out_gray = charRow_gray;
}
                                   ...
                                   int &char_mat_height,int &char_mat_width)
{
    ...
    for(int char_num=0;char_num<charWnd.size();char_num++)
    {
        ...
        if(save)
        {
            const char* single_char_folder_ = "../../../src/data_enhance/char_img";
...
void TextDetector::segmentSrcProject(cv::Mat &spineGray, vector<Mat> &single_char_vec)
{
    ...
    Mat spineGray_copy;
    spineGray_copy = spineGray.clone();
...
//        std::cout<<"mc = "<<mc.x<<" , "<<mc.y<<std::endl;
    ...
    Mat charRow;
    Mat charRow_gray;
    segmentRow(open_src,spineGray_copy,charRow,charRow_gray,0.08);
#ifdef DEBUG
        while(1)
        {
...
//        ++itc;
//    }
    //modify on 0309,for three charters connected
    vector<char_range_t>::iterator itc = char_rang.begin();
    vector<Rect> vecRect_temp;
    if(char_rang.size()>2)
    {
        while ((itc+2) != char_rang.end())
        {
            Rect charRec((*itc).begin,0,(*(itc+2)).end-(*itc).begin,charRow.rows);
            vecRect_temp.push_back(charRec);
            ++itc;
        }
    }
...
//         Mat single_char_=charRow(vecRect.at(char_num));//modify on 0309
        Mat single_char_=charRow_gray(vecRect.at(char_num));//modify on 0309
//         Mat single_char_seg;
